...
\end{itemize}
La figure~\ref{fig.ossia} présente les différents éléments.
...
\begin{itemize}
    ...
    La répartition ne peut dans ce cas fonctionner de manière hiérarchique: chaque exécution sur chaque client aura des temps d'exécution différents pour chaque point d'interaction. 
    ...
    On considère le graphe acyclique non orienté dont les nœuds temporels sont les sommets et les contraintes temporelles sont les arêtes. 
...
%    Un client ne sait pas à l'avance si il va pouvoir se déclencher ou non. 
    ...
    \begin{figure}[h]
        ...
        \begin{subfigure}{0.45\textwidth}\centering
            ...
            \end{tikzpicture}
            \caption{Scénario écrit par le compositeur}
        \end{subfigure}
    
    \begin{subfigure}{0.45\textwidth}\centering
        \begin{tikzpicture}[scale = 0.4,shorten >=1pt,auto,node distance=2 cm]        
        \small
         \node[state,inner sep=1pt, minimum size=0pt] (T0) {$T_0$};
         \node[state,inner sep=1pt, minimum size=0pt] (T1)[above right of=T0] {$T_1$};
         \node[state,inner sep=1pt, minimum size=0pt] (T2)[below right of=T0] {$T_2$};
         \node[state,inner sep=1pt, minimum size=0pt] (T3)[above right of=T2] {$T_3$};
         
         \path (T0) edge[left] node {$C_0$} (T1);
         \path (T0) edge[left] node {$C_1$} (T2);
         \path (T1) edge[left] node {$C_2$} (T3);
         \path (T2) edge[left] node {$C_3$} (T3);
        \end{tikzpicture}
        \caption{Graphe correspondant}
    \end{subfigure}
...
Si l'écriture de scénarios est restreinte à des constructions non-interactives, il est possible d'optimiser le mode compensé décrit précédemment. 
...
\begin{figure}[h]
	\centering
		...
		\end{tikzpicture}
		\captionof{scenario}{Répartition du scénario \ref{scenar.depl-interactif} dans le cas asynchrone instantané : les contraintes associées aux groupes $G_1,G_4$ et la contrainte associée au groupe $G_3$ s'arrêteront et démarreront sans ordre prévisible.}
