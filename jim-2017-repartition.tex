...
\begin{itemize}
    ...
    Cette condition prend la forme d'une expression logique entre paramètres de la partition. Par exemple, 
...
\begin{itemize}
    ...
    Si par exemple on assigne le groupe $G_1$ au scénario~\ref{scenar.general}, tous les clients de $G_1$ vont exécuter tous les éléments, sans communiquer sur leurs résultats. 
...
\begin{itemize}
    \item Si l'exécution du nœud est résolue de manière asynchrone, toutes les machines de $G_2$ envoient l'information de déclenchement à toutes les machines de $G_1$ et $G_3$, sans garantie d'ordre. Les \triggers se déclenchent immédiatement.
    
    Note : si on veut garantir un ordre, ne peut-on pas faire : $G_3$ stoppe $G_1$ qui démarre $B$ ? Ou bien si la borne max est atteinte, $G_1$ stoppe $G_3$ et déclenche $G_2$.
    
    \item Si l'exécution du nœud est résolue de manière synchrone : 
    \begin{itemize}
        \item Si la condition devient vraie, par consensus. On choisit une date, et on fixe l'arrêt des machines de $G_1$ et le départ des clients de $G_2$ par rapport à cette date, calculée en fonction des latences relatives. Potentiellement introduire un tic d'écart entre la fin de $G_1$ et le démarrage de $G_2$ ?
...
\begin{enumerate}
    ...
    Dès qu'une machine de $G_1$ arrivera à $M_1$, le message sera envoyé.
    \item Si on se trouve dans le cas pré-calculé~: la durée calculée sera celle de $C_{1\rightarrow2}$. Les machines du groupe $G_1$ et $G_2$ peuvent être déconnectées après le début de la lecture et continueront à fonctionner comme prévu.
\end{enumerate}
